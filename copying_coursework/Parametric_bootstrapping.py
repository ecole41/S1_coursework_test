    ...
    for toy_id in tqdm(range(Ntoy)):
        ...
        while retries < 5:  #To avoid NAN error vlues, when minuit gets invalid fits -> doesn't converge
            ...
            if np.all(np.isfinite(mi_t.values)) and np.all(np.isfinite(mi_t.errors)):
                ...
                if retries > 0:
                    print(f"Fit succeeded after {retries} retries")
                
                break
            else:
                retries += 1
                print(f"Fit failed, retrying ...")
...
def main():
    ...
    output_file = "Parametric_Bootstrapping_Data/original_values.npy"
    folder_name = "Parametric_Bootstrapping_Data"
    ...
    np.save(output_file, original_values.values)
    print(f"Original values saved to {output_file}")
